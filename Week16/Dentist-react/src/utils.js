...
const generateMailAddress = (name, surname) => {
  const mailAddress = `${lowerCaseWithoutWhitespace(name)}${lowerCaseWithoutWhitespace(surname)}@` + generateMailProvider(mailProvider);
  return mailAddress;
};
...
const arrayOfEmployees = employees.map(employee => {
  ...
  updatedEmployeeInfo.mail = `${lowerCaseWithoutWhitespace(employee.name)}${lowerCaseWithoutWhitespace(employee.surname)}@tandartspraktijkbvt.nl`;

  return updatedEmployeeInfo;
});

// console.log(arrayOfPatients); // 230
// console.log(arrayOfEmployees); // 20
